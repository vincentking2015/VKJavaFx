...
import com.vk.control.tile.events.*;
import com.vk.control.tile.events.TileEvent.EventType;
import com.vk.control.tile.events.TimeEvent.TimeEventType;
import com.vk.control.tile.fonts.Fonts;
import com.vk.control.tile.skins.*;
...
import static com.vk.control.tile.tools.MovingAverage.MAX_PERIOD;

/**
 * 作者:   王恒
 * Author:VINCE
 * 日期:   2020-07-09
 * Des:   来自于https://github.com/HanSolo/tilesfx
 *
 * <p>
 * * #                                                   #
 * #                       _oo0oo_                     #
...
public class Tile extends Control {
    ...
    public void setMinValue(final double VALUE) {
        if (Animation.Status.RUNNING == timeline.getStatus()) { timeline.jumpTo(Duration.ONE); }
        if (null == minValue) {
    ...
    public void setSeries(final List<XYChart.Series<String, Number>> SERIES) {
        SERIES.forEach(series -> addTilesFXSeries(new TilesFXSeries<String, Number>(series)));
    }
    public void setSeries(final XYChart.Series<String, Number>... SERIES) {
        setSeries(Arrays.asList(SERIES));
    }
    public void addSeries(final XYChart.Series<String, Number> SERIES) {
        addTilesFXSeries(new TilesFXSeries<String, Number>(SERIES));
    }
    public void removeSeries(final XYChart.Series<String, Number> SERIES) {
    ...
    public void setFlipTimeInMS(final long FLIP_TIME) { flipTimeInMS = clamp(0, 2000, FLIP_TIME); }
    ...
    public void setMinorTickCount(final int MINOR_TICK_COUNT) { _minorTickCount = clamp(0, 10, MINOR_TICK_COUNT); }
    ...
    public void setMatrixSize(final int COLUMNS, final int ROWS) {
        _matrixSize = new int[] { clamp(2, 1000, COLUMNS), clamp(2, 1000, ROWS) };
        fireTileEvent(RECALC_EVENT);
    }
    ...
    public void setTooltipTimeout(final double TIMEOUT) {
        if (null == tooltipTimeout) {
            _tooltipTimeout = clamp(0, 10000, TIMEOUT);
            fireTileEvent(REDRAW_EVENT);
        } else {
    ...
    public void setBackgroundImageOpacity(final double OPACITY) {
        _backgroundImageOpacity = clamp(0, 1, OPACITY);
    ...
    @Override public String getUserAgentStylesheet() {
        if (null == userAgentStyleSheet) { userAgentStyleSheet = getClass().getResource("/com/vk/control/tile/tilesfx.css").toExternalForm(); }
