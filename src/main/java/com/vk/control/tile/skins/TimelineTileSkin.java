 ...
 */
package com.vk.control.tile.skins;

import com.vk.control.tile.Section;
import com.vk.control.tile.Tile;
import com.vk.control.tile.chart.ChartData;
import com.vk.control.tile.events.TileEvent;
import com.vk.control.tile.fonts.Fonts;
...
import static com.vk.control.tile.tools.Helper.enableNode;
...
public class TimelineTileSkin extends TileSkin {
    ...
    protected void initGraphics() {
        ...
        enableNode(upperUnitText, !tile.getUnit().isEmpty());
        ...
        enableNode(timeSpanText, !tile.isTextVisible());

        stdDeviationArea = new Rectangle();
        enableNode(stdDeviationArea, tile.isAverageVisible());
        ...
        enableNode(thresholdLine, tile.isThresholdVisible());
        ...
        enableNode(lowerThresholdLine, tile.isThresholdVisible());
        ...
        enableNode(averageLine, tile.isAverageVisible());
        ...
        enableNode(sectionGroup, tile.getSectionsVisible());
        ...
        enableNode(percentageInSectionGroup, tile.getSectionsVisible());
    ...
    protected void handleEvents(final String EVENT_TYPE) {
        ...
        if (TileEvent.EventType.VISIBILITY.name().equals(EVENT_TYPE)) {
            ...
            enableNode(valueText, tile.isValueVisible());
            enableNode(unitFlow, !tile.getUnit().isEmpty());
            enableNode(timeSpanText, !tile.isTextVisible());
            enableNode(averageLine, tile.isAverageVisible());
            enableNode(averageText, tile.isAverageVisible());
            enableNode(averageText2, tile.isAverageVisible());
            ...
            enableNode(sectionGroup, tile.getSectionsVisible());
            enableNode(percentageInSectionGroup, tile.getSectionsVisible());
            enableNode(trendText, tile.isTrendVisible());
            redraw();
    ...
    protected void handleCurrentValue(final double VALUE) {
        ...
        if (!reducedDataList.isEmpty()) {
            ...
            path.getElements().clear();
            path.getElements().add(new MoveTo(maxX - (maxTime - data.getTimestamp().getEpochSecond()) * stepX, maxY - Math.abs(minValue - clamp(minValue, maxValue, data.getValue())) * stepY));
            for (long timeSlot = maxTime ; timeSlot >= minTime ; timeSlot -= resolutionStep) {
                if (data.getTimestamp().getEpochSecond() > timeSlot - resolutionStep) {
                    ...
                    dot.setCenterY(maxY - Math.abs(minValue - clamp(minValue, maxValue, data.getValue())) * stepY);
    ...
    private void addData(final ChartData DATA) {
        ...
        int n = clamp(2, reducedDataList.size(), tile.getNumberOfValuesForTrendCalculation());
...
//            (this.getMaxY() + this.getMinY()) * 0.5D
            double centerY = 0.5D*(sections.get(entry.getKey()).getLayoutBounds().getMaxY()
                    +sections.get(entry.getKey()).getLayoutBounds().getMinY());
            double centerY1 = 0.5D*(entry.getValue().getLayoutBounds().getMaxY()
                    +entry.getValue().getLayoutBounds().getMinY());
            entry.getValue().relocate(centerY,centerY1);
//            entry.getValue().relocate(size * 0.05, sections.get(entry.getKey()).getLayoutBounds().getCenterY() - entry.getValue().getLayoutBounds().getCenterY());
    ...
    protected void redraw() {
        ...
        if (tile.getUnit().contains("/")) {
            ...
            enableNode(fractionLine, true);
        } else {
            upperUnitText.setText(" ");
            unitText.setText(tile.getUnit());
            enableNode(fractionLine, false);
        }
