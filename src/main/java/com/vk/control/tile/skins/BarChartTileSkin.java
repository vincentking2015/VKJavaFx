...
import static com.vk.control.tile.events.TileEvent.EventType.*;
...
public class BarChartTileSkin extends TileSkin {
    ...
    protected void initGraphics() {
        ...
        tile.getBarChartItems().forEach(item -> {
            ...
            EventHandler<MouseEvent> clickHandler = e -> tile.fireTileEvent(new TileEvent(SELECTED_CHART_DATA, item.getChartData()));
            ...
            while (change.next()) {
                if (change.wasAdded()) {
                    change.getAddedSubList().forEach(addedItem -> {
                        ...
                        EventHandler<MouseEvent> clickHandler = e -> tile.fireTileEvent(new TileEvent(SELECTED_CHART_DATA, addedItem.getChartData()));
    ...
    protected void handleEvents(final String EVENT_TYPE) {
        ...
        if (VISIBILITY.name().equals(EVENT_TYPE)) {
            Helper.enableNode(titleText, !tile.getTitle().isEmpty());
            Helper.enableNode(text, tile.isTextVisible());
        } else if (DATA.name().equals(EVENT_TYPE)) {
            sortItems();
        } else if (ANIMATED_ON.name().equals(EVENT_TYPE)) {
            tile.getBarChartItems().forEach(item -> item.getChartData().setAnimated(true));
        } else if (ANIMATED_OFF.name().equals(EVENT_TYPE)) {
