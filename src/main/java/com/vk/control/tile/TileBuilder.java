 ...
 */
package com.vk.control.tile;

import com.vk.control.tile.Tile.*;
import com.vk.control.tile.chart.ChartData;
import com.vk.control.tile.chart.RadarChartMode;
import com.vk.control.tile.chart.SunburstChart;
import com.vk.control.tile.chart.SunburstChart.TextOrientation;
import com.vk.control.tile.chart.TilesFXSeries;
import com.vk.control.tile.colors.Bright;
import com.vk.control.tile.events.AlarmEventListener;
import com.vk.control.tile.events.TileEventListener;
import com.vk.control.tile.events.TimeEventListener;
import com.vk.control.tile.skins.BarChartItem;
import com.vk.control.tile.skins.LeaderBoardItem;
import com.vk.control.tile.tools.*;
...
public class TileBuilder<B extends TileBuilder<B>> {
    ...
    public final B timePeriod(final Duration PERIOD) {
        properties.put("timePeriod", new SimpleObjectProperty(PERIOD));
        return (B)this;
    }

    public final B maxTimePeriod(final Duration MAX_PERIOD) {
    ...
    public final B itemSortingTopic(final Tile.ItemSortingTopic ITEM_SORTING_TOPIC) {
    ...
    public final B trackColor(final Tile.TileColor COLOR) {
        properties.put("trackColor", new SimpleObjectProperty(COLOR));
        return (B)this;
    }

    public final B mapProvider(final Tile.MapProvider PROVIDER) {
    ...
    public final B textSize(final Tile.TextSize SIZE) {
    ...
    public final B sunburstVisibleData(final SunburstChart.VisibleData VISIBLE_DATA) {
    ...
    public final Tile build() {
        ...
        if (properties.containsKey("skinType")) {
            ...
            switch (skinType) {
                ...
                case COUNTDOWN_TIMER:
                    TILE.setBarBackgroundColor(TILE.getBackgroundColor().brighter());
                    TILE.setAnimated(false);
                    TILE.setTimePeriod(Duration.ofSeconds(60));
                    break;
                case CYCLE_STEP:
                    break;
                case COLOR:
        ...
        for (String key : properties.keySet()) {
            if ("prefSize".equals(key)) {
                ...
                TILE.setTimePeriod(((ObjectProperty<Duration>) properties.get(key)).get());
            } else if ("maxTimePeriod".equals(key)) {
                TILE.setMaxTimePeriod(((ObjectProperty<Duration>) properties.get(key)).get());
                ...
                TILE.setTextSize(((ObjectProperty<Tile.TextSize>) properties.get(key)).get());
                ...
                TILE.setTrackColor(((ObjectProperty<Tile.TileColor>) properties.get(key)).get());
            } else if ("mapProvider".equals(key)) {
                TILE.setMapProvider(((ObjectProperty<Tile.MapProvider>) properties.get(key)).get());
                ...
                TILE.setItemSortingTopic(((ObjectProperty<Tile.ItemSortingTopic>) properties.get(key)).get());
                ...
                TILE.getSunburstChart().setVisibleData(((ObjectProperty<SunburstChart.VisibleData>) properties.get(key)).get());
